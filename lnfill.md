...
| `parse_fill_log.py` | **F fill log parser** — parses one `fill_YYYYMMDD_HHMM.log` file; skips non-F fills; extracts per-detector fill duration (seconds); looks up GeID from `gsid_to_geid.json`; appends InfluxDB line protocol to `fillTime.txt`. Deployed on both spark-ca9f and pi5-lnFill (2026-04-30) |
| `get_geid_map.py` | **GeID map builder** — queries `GS001_Ge_ID`…`GS110_Ge_ID` via pyepics; saves `gsid_to_geid.json` (GSID→GeID lookup). Run once; re-run after crystal swaps |
| `batch_push_fill.py` | **Batch push** — loops all F fill logs, calls `parse_fill_log.py` for each, pushes `fillTime.txt` to InfluxDB `HPGeTemp` table `FillDuration` |
| `gsid_to_geid.json` | GSID→GeID cache (auto-generated by `get_geid_map.py`; gitignored) |
| `fillTime.txt` | InfluxDB line protocol output (auto-generated; gitignored) |
...
- `LNFill_App.py` writes coarse fill events only (`Fill,type=<X> value=1/0`). **Per-detector fill durations are NOT in InfluxDB** — text logs only (`logs/fill_YYYYMMDD_HHMM.log`). This is the key gap for any future LN2 fill time monitoring.
...
- `LNFill_App.py` writes only coarse fill events to InfluxDB: `Fill,type=<X> value=1` at start and `Fill,type=<X> value=0` at end (via `WriteInflux()`) ✅ verified 2026-04-13 — `LNFill_App.py:L259,L532`. **Per-detector fill durations are NOT written to InfluxDB** — they exist only in text log files (`logs/fill_YYYYMMDD_HHMM.log`). This is the key gap for any future LN2 fill time monitoring.
...
- [`lnfill_ioc.md`](lnfill_ioc.md) — LN2 fill system deep internals: InfluxDB data flow, hose→detID mapping table, ln2con IOC boot tree, DetMan.py FillManifold() state machine
- [`con6_lnfill.md`](con6_lnfill.md) — con6 (Solaris 10): CVS source repo + 68040 cross-compiler for lnfill IOC; archiving plan
- [`influxdb_grafana.md`](influxdb_grafana.md) — InfluxDB/Grafana on DCS2; HPGeTemp database written by `SaveTemp.sh` / `StoreDetTemps.py`
- [`collectorboxpi.md`](collectorboxpi.md) — Raspberry Pi soft IOC for HV control; parallel Pi infrastructure
- [`hardware_architecture.md`](hardware_architecture.md) — LN2 system role in detector cooling chain
- [`troubleshooting.md`](troubleshooting.md) — LN2-related operational issues
