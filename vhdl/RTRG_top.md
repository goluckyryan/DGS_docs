...
1. **ROUTER_DATA_PATH (U8)**: Receives SERDES data from 8 digitizers; runs 8 `chan_in` instances; builds `LINKL_RAW_DATA[15:0]`:
   - `[15]` = ANY_THROTTLE_REQUEST
   - `[14:8]` = Y-plane sum (7-bit, max 80)
   - `[7]` = DATA_VALID (all digitizers + router locked)
   - `[6:0]` = X-plane sum (7-bit, max 80)
...
2. **F12_DECODE process**: When FRAME_12_REQ_FLAG asserted:
   - `FRAME_12_DATA(2)` → `MASTER_COUNTER_RESETS`
   - `FRAME_12_DATA(3)` → `MASTER_FIFO_CLEARS`
3. **ADD_VETOES_BLK (for i in 1 to 8)**: Per-digitizer veto insertion state machine.
   - Normal words: pass `SANITIZED_CONTROL_DATA` unchanged; accumulate `LATCHED_CHANNEL_VETOES(i) |= CHANNEL_VETOES(i)`
   - 5th word of frames 1–19 (WORD_INDEX = 5, 10, 15, …, 95): Replace outgoing word with `"000000" & LATCHED_CHANNEL_VETOES(i)[9:0]`; then reset latch to current live CHANNEL_VETOES(i)
   - (5th word of frame 20 is reserved for machine sync — not overridden)
...
- `TOTAL_COARSE_GE_SUM[5:0]` (6-bit count of coarse Ge hits across all 8 digitizers) from ROUTER_DATA_PATH
- Sent to CPLD via **DDR ODDR primitives** on SUMCOPY[3:1]:
  - ODDR instance i (0..2): D1 = `TOTAL_COARSE_GE_SUM(i)`, D2 = `TOTAL_COARSE_GE_SUM(i+3)`
...
- `SUMCOPY[0]` = `xLOC_ADDR(2)` (VME address bit for CPLD register decode)
