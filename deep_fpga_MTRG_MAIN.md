...
| 0x0814 | MISC_CLK_CTL_REG — miscellaneous clock control (power-up: 0x0007). **Bit 15:** set '1' to request Link L clock as master clock. Hardware-enforced fallback: if Link L SERDES is not locked, firmware automatically uses oscillator regardless of this bit. ✅ verified 2026-04-25 — `top.vhd:L1095` (`xCLK_SRC_SEL <= '0' when ((MISC_CLK_CTL_REG(15) = '1') and (xLINK_LOCK(9) = '0')) else '1'`; comment: `'0' => INB (LINKL_RCLK)  '1' => INA (oscillator)`). Bits 0–5,8–13: ICS581 test/FS input controls for U50/U53/U54. |
| 0x0840 | MISC_CTL1_REG. **Bit 6:** set '1' to request Imperative Sync (ImpSync) broadcast. MTRG sends command byte 0x81 (vs normal 0x01) in Frame 1 to reset all board timestamps. Released automatically after issuance. ✅ verified 2026-04-25 — `top.vhd:L1105` (`IMPERATIVE_SYNC_REQUEST <= ... else MISC_CTL1_REG(6)`); `mstr_mach.vhd:L340` (ImpSync sends 0x81 in Frame 1). Other bits: bit 0=RETRY (link init reset), bit 1=LOCK_ACK, bit 2=RST, bits 3–5=STRINGENT_LOCK_FLAG for links L/R/U. |
| 0x0844 | MISC_CTL2_REG. See RTRG_support_modules.md for bit map (throttle, frame count). |
| 0x08D0 | LINK_L_PROPAGATION_CONTROL_REG — controls which TTCL frames propagate from/to Link L (power-up: 0x0000). Bit 0: F1 propagation (clock/sync). Bits for F3–F7: trigger propagation. ✅ verified 2026-04-25 — `registers.vhd:L1169` (reset value 0x0000; label LINK_L_PROPAGATION_CONTROL_REG). |
| 0x08D4 | LINK_R_PROPAGATION_CONTROL_REG — same as 0x08D0 but for Link R (power-up: 0x0000). ✅ verified 2026-04-25 — `registers.vhd:L1170`. |
| 0x08D8 | LINK_U_PROPAGATION_CONTROL_REG — same as 0x08D0 but for Link U (power-up: 0x0000). ✅ verified 2026-04-25 — `registers.vhd:L1171`. |
...
- [fpga.md](fpga.md) — System-level overview: trigger hierarchy, 20-frame cycle, end-to-end timeline
- [deep_fpga_MTRG.md](deep_fpga_MTRG.md) — MTRG overview: device layout (Main FPGA + VME FPGA + CPLD)
- [deep_fpga_MTRG_VME.md](deep_fpga_MTRG_VME.md) — VME FPGA: A32/D32 VME slave, FPGA config manager
- [deep_fpga_MTRG_CPLD.md](deep_fpga_MTRG_CPLD.md) — CPLD: fast strobe multiplicity logic (~1 µs latency)
- [deep_fpga_MTRG_VIVADO.md](deep_fpga_MTRG_VIVADO.md) — Vivado port: Kintex UltraScale XCK060 differences
- [deep_fpga_RTRG.md](deep_fpga_RTRG.md) — RTRG firmware: multiplicity data the MTRG receives on Link L
- [ttcl.md](ttcl.md) — TTCL spec: 20-frame downstream command structure generated by the MTRG
- [myriad.md](myriad.md) — MγRIAD auxiliary detector interface (connects to MTRG trigger algorithms)
- [connectors.md](connectors.md) — MTRG connector pinouts: 125-pin SERDES, NIM I/O, CPLD ribbons, ECL
- [vhdl/MTRG_top.md](vhdl/MTRG_top.md) — `top.vhd` analysis: 8-Router aggregation, trigger decision distribution, NIM I/O, CPLD bus wiring
- [vhdl/MTRG_eight_mt_channel.md](vhdl/MTRG_eight_mt_channel.md) — `eight_mt_channel.vhd` analysis: instantiates 8 `mt_input_channel` blocks, one per Router link
- [vhdl/MTRG_mt_input_channel.md](vhdl/MTRG_mt_input_channel.md) — `mt_input_channel.vhd` analysis: per-Router SERDES receiver, hit extraction, multiplicity contribution
- [vhdl/MTRG_sum_hits_X.md](vhdl/MTRG_sum_hits_X.md) — `sum_hits_X.vhd` analysis: north/south hemisphere clean-hit aggregation
- [vhdl/MTRG_calc_total_sum.md](vhdl/MTRG_calc_total_sum.md) — `calc_total_sum.vhd` analysis: final multiplicity sum and trigger decision comparator
- [vhdl/MTRG_MYRIAD_RCV_MACH.md](vhdl/MTRG_MYRIAD_RCV_MACH.md) — `MYRIAD_RCV_MACH.vhd` analysis: MγRIAD receiver state machine, 5-word SERDES frame lock, NIM/ECL/FERA state extraction
- [vhdl/MTRG_MYRIAD_TRIGGER.md](vhdl/MTRG_MYRIAD_TRIGGER.md) — `MYRIAD_TRIGGER.vhd` analysis: MγRIAD trigger algorithm, programmable delay line, coincidence modes, subtypes 0x78/0x79
- [multi_system_linking.md](multi_system_linking.md) — Cross-system clock/trigger sharing via Link L/R/U; MTRG ImpSync (0x840 b6) and clock-source selection (0x814 b15) are key registers in multi-system setups
- [vhdl/MTRG_mstr_mach.md](vhdl/MTRG_mstr_mach.md) — `mstr_mach.vhd` analysis: Master State Machine, 20-frame TTCL output cycle, FIFO pipelining, monitor controls
- [vhdl/MTRG_local_trig_coinc.md](vhdl/MTRG_local_trig_coinc.md) — `local_trig_coinc.vhd` analysis: local-vs-local coincidence trigger algorithm, dual-mask OR, overlap window
- [vhdl/MTRG_trig_algo_support.md](vhdl/MTRG_trig_algo_support.md) — `trig_algo_support.vhd` analysis: shared base component for all trigger algorithms (dual FIFO, prescaler, holdoff, throttle)
- [vhdl/MTRG_support_modules.md](vhdl/MTRG_support_modules.md) — support/infrastructure modules: timestamp counter, TDC vernier compressor, SERDES fan-out, cross-system trigger, monitor FIFO collector, type definitions
- [vhdl/MTRG_registers.md](vhdl/MTRG_registers.md) — `registers.vhd` analysis: complete VME register map (~120 registers, 3 lookup RAMs, 8+8 monitor FIFOs, VME FSM)
- [vhdl/MTRG_AUX_IO.md](vhdl/MTRG_AUX_IO.md) — `AUX_IO.VHD` analysis: AUX_A/B port mux, NIM output modes, target wheel encoder (parallel + SSI), BEAM_SWEEP_OUT, SSI serial receiver FSM
- [vhdl/MTRG_SERDES_RX_Mach.md](vhdl/MTRG_SERDES_RX_Mach.md) — `SERDES_RX_Mach_R2.vhd` analysis: 20-frame FSM, frame decoders (Sync/ISY, triggers F3–F10, spare/cmd F11–F19, EOC), VETO_EVENT, sanitized output
- [vhdl/MTRG_comp_defs.md](vhdl/MTRG_comp_defs.md) — `trigger_comp_defs.vhd` + `trigger_top_comp_defs.vhd`: all component declarations, tdc_chain_cont ports
- [vhdl/MTRG_pos_finder.md](vhdl/MTRG_pos_finder.md) — `pos_finder.vhd` analysis: 11/12-bit thermometer-to-4-bit edge position lookup (2048-entry ROM, 1-cycle pipeline)
- [vhdl/MTRG_sum_hits_XY.md](vhdl/MTRG_sum_hits_XY.md) — `sum_hits_XY.vhd` analysis: XY coincidence trigger (fires when both X+Y global sums exceed threshold, 2-state FSM)
- [vhdl/MTRG_link_init_and_input_pipeline.md](vhdl/MTRG_link_init_and_input_pipeline.md) — `link_init.vhd` + `mstr_trigger_input_pipeline.vhd` analysis: 7-state SERDES link init FSM (INIT→EN_SERDES→SYNC→WAIT_FOR_LOCK→ALL_LOCKED_UP→ACKED→ERROR), LINK_MASK VME register; 828-line per-Router data unpacker (preamble lock, 8×12-word blocks, 132-bit TEMP_REG, hit pattern/CC energy/crystal ID, GROUP_ENERGY_SUM, DIGVALID gate, throttle aggregation)
- [vhdl/MTRG_GITMO.md](vhdl/MTRG_GITMO.md) — `GITMO_RCV_MACH.vhd` + `GITMO_TRIGGER.vhd` analysis: GITMO legacy VXI interface (Link L 2-stage prelock, 5-word frame, NIM/ECL/FERA/RDY_BSY/EOE decoding), GITMO_TRIGGER 5-state FSM (type 0x56, 20 ns configurable delay)
- [vhdl/MTRG_tdc_chain_cont.md](vhdl/MTRG_tdc_chain_cont.md) — `tdc_chain_cont.vhd` analysis: 4-phase carry-chain TDC (tdc_unit_cont ×4), 5-state autosample FSM, 8-word TDC event packet format
- [vhdl/MTRG_Generated_top.md](vhdl/MTRG_Generated_top.md) — `Generated_top.vhd` analysis: top-level `trigger_top` entity (6,286 lines), all 24 component instances, trigger algo slot map, veto system, 8 monitor FIFOs, inline logic inventory
- [vxworks_trigger_drivers.md](vxworks_trigger_drivers.md) — VxWorks IOC trigger asyn drivers: `asynTrigMasterDriver` (MTRG, 369 params, card init from reg 0x15C), `asynTrigRouterDriver` (RTRG, 188 params), firmware type code table, boot sequence
...
- [trig_sys_sim.md](trig_sys_sim.md) — MTRG trigger system simulation testbench: two `trigger_top` (BUILD_TYPE=4) wired via 79 ns fake SERDES link, VME bus mux model, 8-step stimulus sequence (link init → manual triggers → propagation → DC balance → rapid-fire), full VME register address constants, BUILD_TYPE code map 0–F
