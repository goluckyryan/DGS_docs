...
| `gtReceiver` | **Legacy C++ data receiver** (pre-ANLDAQ), v6.57 by M. Oberling. Connects to IOC TCP server, receives raw binary data, writes GEB-format files. Key SOE markers: `DIG_SOE=0xAAAAAAAA` (digitizer), `TRIG_SOE=0xAAAA0000` (trigger, upper 16 bits only). GEB types written: 14=`GEB_TYPE_DGS`, 15=`GEB_TYPE_DGSTRIG`. Build flags (compile-time): `WRITEGTFORMAT` (GEB header output, always on), `FILE_PER_CHANNEL` (one file per board×channel — active default), `FOLDER_PER_RUN` (subdirectory per run, active), `DUMP_UNKNOWN_DATA_TO_DISK` + `DEBUG_OUTPUT_FILE` (trigger/unknown data to `_DIAG` file, active). Limits: `MAXBOARDID=4095`, `MAXCHID=16`, `DIG_MIN_HEADER_LENGTH=3 words`, `TRIG_MIN_HEADER=8 words`, `REFORMATTED_HEADER=10 words`. Summary printed every 5s (`SUMMARY_OUTPUT_INTERVAL`). Poll interval 10µs (`MAXNS=10000`). Utility tools: `rcvr_merge` — k-way timestamp-ordered GEB merge (reads all matching files in CWD by prefix, outputs one merged GEB file; `argv[1]`=output, `argv[2]`=filename prefix; up to 1024 files, 512KB I/O buffer); `rcvr_unmerge` — splits merged GEB file back into per-type files; `rcvr_data_scrubber` — scans GEB file for corrupt/malformed events (checks SOE word, header length, waveform sanity; `TOTAL_HEADER_LENGTH_WORDS=13`). Full GEB type table (types 1–23) in `dgs_analysis.md`. Superseded by `ANLDAQ/tcpReceiverMT`. |
